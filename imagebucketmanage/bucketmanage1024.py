...
class BucketManager:
    def __init__(self, dic_data, valid_ids=None, max_size=(1024,1024), divisible=64, step_size=8, min_dim=256, base_res=(1024,1024), bsz=1, world_size=1, global_rank=0, max_ar_error=4, seed=42, dim_limit=1024, debug=False):
    ...
    def assign_buckets(self):
        ...
        if self.debug:
            ...
            for bucket_id in reversed(sorted(self.buckets.keys(), key=lambda b: len(self.buckets[b]))):
                print(f"bucket {bucket_id}: {self.resolutions[bucket_id]}, aspect {self.aspects[bucket_id]:.5f}, entries {self.buckets[bucket_id]}")
...
if __name__ == "__main__":
    ...
    image_folder = r"C:\abc"  # 图像文件夹的实际路径
    image_class_folder_out = r"C:\Users\efg"  # 图像分类文件夹的输出路径
    file_index = 10000

    for filename in os.listdir(image_folder):
        if filename.endswith(".jpg") or filename.endswith(".png"): #在这里改或增加后缀
            file_path = os.path.join(image_folder, filename)
            image = Image.open(file_path)
            resolution = image.size
            image_data[str(file_index)] = resolution
            dic_file_name[str(file_index)] = filename
            file_index+=1

    bm = BucketManager(image_data, debug=False,divisible=64,max_ar_error=1,dim_limit=2048) #调整这些参数就能控制buckets的生成数量，每个bucket中的图片数量
    for bucket_id in reversed(sorted(bm.buckets.keys(), key=lambda b: len(bm.buckets[b]))):
        for file_index in bm.buckets[bucket_id]:
            file_src_path = os.path.join(image_folder,dic_file_name[file_index])
            out_path = os.path.join(image_class_folder_out,f"{bm.resolutions[bucket_id][0]}x{bm.resolutions[bucket_id][1]}")
            if not os.path.exists(out_path):
                os.makedirs(out_path)
            shutil.copy(file_src_path,out_path) #copy可换成move,即移动文件到指定文件夹
            print(f"fileIndex:{file_index},fileName:{dic_file_name[file_index]},reso:{bm.resolutions[bucket_id]}")
